...
def get_word():
    return C(word_list)  # Просто C, без .choice()
...
def play_word(word):
    ...
    while tries > 0:
        ...
        if word_input in guessed_words or word_input in guessed_letters:
            print('Вы уже вводили это.')
            continue

        if len(word_input) > 1:  # Угадали слово целиком?
            if word_input == word:
